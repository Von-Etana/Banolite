      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {filteredProducts.map((product, index) => (
            ...
            >
              <Link href={`/book/${product.id}`} className="aspect-square md:aspect-[4/5] rounded-lg md:rounded-xl mb-2.5 md:mb-4 relative overflow-hidden flex items-center justify-center cursor-pointer bg-gray-100 shadow-inner">
                ...
                <div className="absolute top-2 left-2 md:top-3 md:left-3 bg-brand-dark text-white shadow-md text-[9px] md:text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 md:px-2.5 md:py-1 rounded-md">
                  {product.type}
                </div>
                ...
                <div className="absolute inset-0 bg-brand-dark/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <div className="bg-white px-3 py-1.5 md:px-4 md:py-2 rounded-lg text-[10px] md:text-xs font-bold flex items-center gap-1.5 shadow-xl text-brand-dark transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                    <Eye className="w-3 md:w-3.5 h-3 md:h-3.5" /> View
                  </div>
                </div>
              </Link>

              <div className="px-1 pb-0.5 md:pb-1 flex-1 flex flex-col">
                <Link href={`/book/${product.id}`} className="block">
                  <h3 className="font-bold text-sm md:text-base text-gray-900 mb-1 md:mb-1.5 hover:text-brand-primary transition-colors line-clamp-1">{product.title}</h3>
                </Link>
                <p className="text-gray-500 text-xs md:text-sm mb-2 md:mb-4 line-clamp-1 md:line-clamp-2 leading-relaxed">{product.description}</p>

                <div className="mt-auto pt-2 md:pt-4 border-t border-gray-100 flex items-center justify-between gap-2 md:gap-4">
                  <span className="text-[10px] md:text-xs font-semibold text-gray-500 uppercase tracking-wide truncate">by {product.creator.split(' ')[0]}</span>
                  <button
                    onClick={() => addToCart(product)}
                    className="p-1.5 md:p-2.5 rounded-lg md:rounded-xl bg-gray-50 text-gray-700 hover:bg-brand-primary hover:text-white hover:shadow-lg hover:shadow-brand-primary/20 transition-all border border-gray-200 hover:border-brand-primary flex-shrink-0"
                    title="Add to Cart"
                  >
