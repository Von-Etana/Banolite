    ...
    return (
        <div className="min-h-screen pt-28 pb-20 bg-gray-50">
            <div className="container mx-auto px-6">
                ...
                {/* Navigation Tabs */}
                <div className="flex items-center gap-3 border-b-0 mb-10 overflow-x-auto pb-2 custom-scrollbar">
                    <button
                        onClick={() => setActiveTab('purchases')}
                        className={`px-6 py-3 rounded-full font-bold text-sm transition-all whitespace-nowrap flex items-center gap-2 ${activeTab === 'purchases' ? 'bg-brand-dark text-white shadow-elevated' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 hover:border-gray-300'}`}
                    >
                        Digital Goods <span className={`py-0.5 px-2 rounded-full text-xs font-bold ${activeTab === 'purchases' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-500'}`}>{purchasedProducts.length}</span>
                    </button>
                    <button
                        onClick={() => setActiveTab('bookings')}
                        className={`px-6 py-3 rounded-full font-bold text-sm transition-all whitespace-nowrap flex items-center gap-2 ${activeTab === 'bookings' ? 'bg-brand-dark text-white shadow-elevated' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 hover:border-gray-300'}`}
                    >
                        Coaching Sessions <span className={`py-0.5 px-2 rounded-full text-xs font-bold ${activeTab === 'bookings' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-500'}`}>{userBookings.length}</span>
                    </button>
                    <button
                        onClick={() => setActiveTab('tickets')}
                        className={`px-6 py-3 rounded-full font-bold text-sm transition-all whitespace-nowrap flex items-center gap-2 ${activeTab === 'tickets' ? 'bg-brand-dark text-white shadow-elevated' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 hover:border-gray-300'}`}
                    >
                        Event Tickets <span className={`py-0.5 px-2 rounded-full text-xs font-bold ${activeTab === 'tickets' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-500'}`}>{userTickets.length}</span>
                    </button>
                </div>
                        ...
                        {userBookings.length === 0 ? (
                            ...
                            userBookings.map((booking: any) => (
                                <div key={booking.id} className="bg-white rounded-2xl p-6 border border-gray-100 hover:shadow-lg transition-all flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    ...
                                    <div>
                                        {booking.meetLink ? (
                                            <a href={booking.meetLink} target="_blank" rel="noopener noreferrer" className="px-6 py-3 bg-brand-primary text-white rounded-full font-bold shadow-elevated hover:bg-brand-primary/90 transition-all inline-flex items-center gap-2">
                            ...
                            userTickets.map((ticket: any) => (
                                <div key={ticket.id} className="bg-white rounded-3xl overflow-hidden border border-gray-200 shadow-sm relative group">
                                    ...
                                    <div className="p-6 pt-8">
                                        ...
                                        <div className="bg-gray-50 rounded-2xl p-6 mb-6 border border-gray-100 border-dashed">
                                            <div className="text-center w-full">
                                                <div className="w-full h-32 bg-white rounded-xl shadow-sm flex flex-col items-center justify-center p-2 mb-3">
                                                    {/* Fake QR representation */}
                                                    <div className="grid grid-cols-4 grid-rows-4 gap-1.5 w-20 h-20 opacity-80">
                                                        {Array.from({ length: 16 }).map((_, i) => (
                                                            <div key={i} className={`${Math.random() > 0.4 ? 'bg-brand-dark' : 'bg-transparent'} rounded-sm`}></div>
                                                        ))}
                                                    </div>
                                                </div>
                                                <p className="font-mono text-xs font-bold text-gray-400 tracking-widest bg-white inline-block px-3 py-1 rounded-md border border-gray-100">{ticket.qrCode}</p>
                                            </div>
                                        </div>
                                    ...
                                    <div className="px-6 py-4 bg-gray-50 border-t border-gray-100">
                                        <button className="w-full py-3.5 bg-white border border-gray-200 text-brand-dark rounded-full font-bold text-sm hover:bg-gray-100 hover:shadow-subtle transition-all flex items-center justify-center gap-2">
